...
- Project Agent 任务详情集中在任务入口：预览原因、目标资源、命令数、版本、编辑
  等待、失败信息与完成后的资源入口由任务详情展示。会话右侧上下文不再重复列出
  “项目任务”；独立的“发布到工作区”请求仍留在右侧。
- 项目邀请、访问申请和文件请求通过通知提醒相关用户；仍需用户处理或等待结果的
  事项保留在任务面板。项目邀请的通知入口指向对应任务，接受、拒绝或撤回后通知
  转为已读，任务按原状态机收敛。
- 固定的 Project 会话排在所属项目的左侧会话列表最前，并使用亮色标识；取消固定后
  恢复原有排序。固定状态属于当前用户，图钉操作成功后立即刷新列表。
