...
- 文件夹按 ID 的 UTF-16 编码单元升序；文档按更新时间倒序、相同时间按 ID 同序升序。
  游标携带受加密认证保护的排序版本，旧排序游标返回 `cursor_stale`，客户端必须从第一页
  重启；跨请求不提供候选集快照。
...
- Markdown 往返比较按块类型规范化明确的编辑器默认值：段落/列表的
  `collapsed: false`、普通列表的 `checked: false`、列表的 `order: null`、
  便签的 `lockedBySelf: false` 与完整默认 `edgeless` 样式可与缺省值等价。
  非默认值、未知属性、评论及不能无损转换的结构仍拒绝，不按属性前缀批量忽略。
- 段落、列表和代码块的四个已知 `meta:createdAt/By`、`meta:updatedAt/By` 字段
  经类型检查后不参与正文往返比较。直接更新继续使用结构差异增量：保留存续块的
  创建记录及未修改块的修改记录；修改块的修改记录、新增块的创建/修改记录由
  服务端使用当前操作用户与时间维护，和正文在同一事务中提交。不为旧块补造缺失
  的创建历史；无变化请求和幂等重放不改写这些记录。
- 原生更新结果应用前，按同级块的正文语义匹配未修改块，保留其块 ID 和 `Y.Text`
  对象，再处理原生匹配的修改块及新增/删除块，避免解析器文本分段或默认属性差异
  将未修改条目的身份分配给新内容。Markdown 往返检查、执行时 ACL/版本检查和
  写后验证继续生效。
...
普通 Workspace 文档的客户端同步广播和服务端/outbox 广播统一在投递时过滤接收者：
身份来自服务端认证上下文，检查用户有效、`Workspace.Sync` 和 `projectScope(null)` 下的
`Doc.Read`。目录表另保留完整表读取权限检查。两种同步协议均遵守此规则；权限决定仅在
单次广播内按用户复用。权限检查与网络发送不是原子操作，后续撤权无法收回已合法发送的数据。

直接资源的页面/画布类型取自 `docProperties.primaryMode`，当前非空值优先于根文档旧版
属性，缺省为页面。正文版本包含有效模式；画布正文替换返回 `unsupported_document_kind`，
读取后切换模式的旧版本请求返回 `version_conflict`。正文事务按根文档、正文、当前属性表
顺序读取并持有既有内容锁，覆盖旧版与当前模式来源；Web 同步和快照写入也使用相同文档的
内容锁，模式变更与正文写入按提交顺序串行化。标题修改及 Markdown 结构限制保持原契约。
文档列表复用 Repeatable Read 只读观察，不取得正文/属性表写锁，避免多文档遍历引入反向锁序；
该快照只覆盖单次请求。
