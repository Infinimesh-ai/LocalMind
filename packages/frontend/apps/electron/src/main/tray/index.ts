...
class TrayState implements Disposable {
  ...
  getSecondaryMenuProvider(): TrayMenuProvider {
    return {
      ...
      getConfig: () => [
        {
          label: 'Open LocalMind',
          click: () => {
            logger.info('User action: Open LocalMind');
            activateMainWindow();
          },
        },
        {
        ...
        'separator',
        {
          label: 'Quit LocalMind Completely...',
          click: () => {
            logger.info('User action: Quit LocalMind Completely');
            app.quit();
          },
        },
      ],
