ALTER TABLE "ai_mcp_delegation_requests"
  ...
  ADD COLUMN "plan_fingerprint" VARCHAR;
...
BEGIN
  ...
  IF jsonb_typeof(plan) <> 'object' OR
     ...
     COALESCE(plan->>'kind', '') NOT IN ('answer', 'document_update', 'unsupported_task') OR
...
ALTER TABLE "ai_mcp_delegation_requests"
  ADD CONSTRAINT "ai_mcp_delegation_requests_plan_shape_check" CHECK (
    ai_mcp_delegation_plan_is_valid("plan_snapshot") AND
    (
      ("plan_snapshot" = '{}'::jsonb AND "plan_fingerprint" IS NULL) OR
      (
        "plan_snapshot" <> '{}'::jsonb AND
        "plan_fingerprint" IS NOT NULL AND
        length(btrim("plan_fingerprint")) BETWEEN 8 AND 128
      )
    )
  );

CREATE OR REPLACE FUNCTION ai_mcp_delegation_requests_plan_update_restrict()
RETURNS TRIGGER AS $$
BEGIN
  IF OLD.plan_snapshot <> '{}'::jsonb AND (
    NEW.plan_snapshot IS DISTINCT FROM OLD.plan_snapshot OR
    NEW.plan_fingerprint IS DISTINCT FROM OLD.plan_fingerprint
  ) THEN
    RAISE EXCEPTION 'ai_mcp_delegation_requests_plan_update_restrict_check';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER ai_mcp_delegation_requests_plan_update_restrict_check
  BEFORE UPDATE OF plan_snapshot, plan_fingerprint
  ON "ai_mcp_delegation_requests"
  FOR EACH ROW
  EXECUTE FUNCTION ai_mcp_delegation_requests_plan_update_restrict();
