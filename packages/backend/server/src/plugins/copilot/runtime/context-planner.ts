...
export const CONTEXT_PLANNER_STRATEGY_VERSION = 'context-planner/v4';
...
function selectMemories(
  ...
  for (const memory of [...memories].sort((left, right) => {
    const score =
      memoryRelevance(right, latestUserMessage) -
      memoryRelevance(left, latestUserMessage);
...
function buildContextMessage(
  ...
  const available = Math.max(
    0,
    CONTEXT_MAX_CHARS - trustBoundary.length - 2 - separatorSize
  );
...
export class ContextPlanner {
  ...
  private planBounded(
    ...
    const memories = selectMemories(input.memories ?? [], latestUserMessage);
    ...
    return { messages: rendered, checkpoint, diagnostics };
  }
