...
export class IntelligenceWorkbenchAuthorizationModel extends BaseModel {
  ...
  async projectCopyPermission(input: ProjectDocumentRef & { actorId: string }) {
    ...
    return this.inspectProjectCopyPermission(input);
  }

  // Display-only inspection after the caller checks membership and personal read access.
  // Copy operations must use projectCopyPermission to serialize authorization changes.
  async inspectProjectCopyPermission(
    input: ProjectDocumentRef & { actorId: string }
  ) {
  ...
  async getAccessRequest(id: string) {
    return await this.db.accessRequest.findUnique({
      where: { id, purpose: 'project_copy', beneficiaryType: 'project' },
      include: {
  ...
  async listAccessRequests(input: {
    ...
    const ids = await this.db.$queryRaw<Array<{ id: string }>>(Prisma.sql`
      ...
      WHERE request.workspace_id = ${workspaceId}
        AND request.purpose = 'project_copy'
        AND request.beneficiary_type = 'project'
        ${documentFilter}
        ${statusFilter}
        AND (
          EXISTS (
  ...
  async approveAccessRequest(input: {
    ...
    if (current.beneficiaryType === 'project' && current.beneficiaryProjectId) {
      ...
      const existingGrant = await this.findActiveProjectGrant({
  ...
  async removeSourceDocumentAuthorizations(input: {
    ...
    const projectIds = (
      await this.db.aiContextProjectGrant.findMany({
    ...
    const pendingRequests = await this.db.accessRequest.findMany({
      where: { workspaceId, docId, status: 'pending', purpose: 'project_copy' },
      orderBy: { id: 'asc' },
    });
