...
mod tests {
  ...
  fn should_parse_openai_chat_stream_with_legacy_function_call() {
    ...
    assert!(parsed.iter().any(|event| matches!(event, StreamEvent::Error { .. })));
    assert!(!parsed.iter().any(|event| matches!(event, StreamEvent::ToolCall { .. })));
  }
  ...
  fn should_encode_responses_stream_with_tool_result_citation_and_error() {
    ...
    assert!(
      reparsed
        .iter()
        .any(|event| matches!(event, StreamEvent::Error { code: Some(code), .. } if code == "response.error"))
    );
  }
  ...
  fn should_encode_chat_stream_with_citation_and_error() {
    ...
    assert!(
      reparsed
        .iter()
        .any(|event| matches!(event, StreamEvent::Error { code: Some(code), .. } if code == "upstream_error"))
    );
  }
