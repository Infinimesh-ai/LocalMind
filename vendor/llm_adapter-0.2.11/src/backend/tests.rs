...
#[cfg(feature = "schema")]
#[test]
fn should_reject_schema_invalid_object_before_route_success() {
  let client = MockHttpClient::with_json_responses(vec![MockHttpResponse::Json(Ok(HttpResponse {
    status: 200,
    body: json!({
      "id": "resp_struct_1",
  ...
  assert!(matches!(response, BackendError::InvalidStructuredOutput { .. }));
  assert!(!response.to_string().contains("summary"));
}
