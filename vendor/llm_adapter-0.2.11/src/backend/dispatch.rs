...
impl StructuredProtocol {
  ...
  fn decode_structured_response(&self, body: &Value) -> Result<StructuredResponse, BackendError> {
    ...
    let output_json = parse_structured_output(&output_text).ok_or_else(|| BackendError::InvalidStructuredOutput {
      message: "structured response did not contain valid JSON".to_string(),
    })?;
...
pub fn dispatch_structured_request(
  ...
  let decoded = protocol.decode_structured_response(&response.body)?;
  #[cfg(feature = "schema")]
  {
    let validator = jsonschema::validator_for(&request.schema).map_err(|_| BackendError::InvalidStructuredOutput {
      message: "invalid structured response schema".to_string(),
    })?;
    let value = decoded
      .output_json
      .as_ref()
      .ok_or_else(|| BackendError::InvalidStructuredOutput {
        message: "structured response did not contain valid JSON".to_string(),
      })?;
    if !validator.is_valid(value) {
      return Err(BackendError::InvalidStructuredOutput {
        message: "structured response schema validation failed".to_string(),
      });
    }
  }
  Ok(decoded)
